...
<div class="product__single-item-info mb-3 limit {{$vSettings->type}}"
     ...
     data-req="{{ $vSettings->is_required }}" data-id="{{ $vSettings->id }}"
     data-limit="{{ ($discountFirst)?$discountFirst->qty:0 }}"
            ...
            @foreach($discounts as $discount)
                ...
                <div
                    ...
                    <label class="custom-label"
                        ...
                        <span
                            class="font-sec-ex-light font-26 count">any {{ $discount->qty }} for {{ convert_price($discount->price,get_currency(), false) }}</span>
                    </label>
                </div>
            @endforeach
        </div>
