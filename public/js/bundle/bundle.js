...
$(document).ready(function () {
  ...
  productSlider.products(); // product-slider
...
$("body").find(".products__item-wrapper").hover(function () {
  ...
  $(this).closest('.products-box').css('height', productsWallHeight); // $(this).closest('.products__item-wrapper').find('.products__item-wrapper-inner').css('height',567);
}, function () {
...
$(document).ready(function () {
  ...
  $("#singleProductPageCnt").fadeIn(function () {
    ...
    var setOfferPrice = function setOfferPrice(offer, offerPrice) {
      ...
      offer.html("".concat(getCurrencySymbol()).concat(offerPrice));
    };
    ...
    var countOfferTotalPrice = function countOfferTotalPrice() {
      ...
      $('.offer-total-price').html("".concat(getCurrencySymbol()).concat(offer_total_price));
    };
    ...
    var countOfferPrice = function countOfferPrice() {
      ...
      $('#specialPopUpModal .special__popup-main-product-item').each(function () {
        ...
        setOfferPrice($(this).find('.product__single-item_price'), value); // var addedPricePlace = $(`#specialPopUpModal .added-offers .special__popup-content-right-product[data-id="${id}"] .special__popup-content-right-product-price`);
    ...
    var setTotalPrice = function setTotalPrice(totalPrice) {
      // console.log(totalPrice, 222222222222222222);
      totalPrice !== undefined && $('.continue-shp-wrapp .price-place-summary').html("".concat(getCurrencySymbol()).concat(Number(totalPrice).toFixed(2)));
    };
    ...
    $('body').on('click', '.product__single-item-info-bottom .inp-up, .product__single-item-info-bottom .inp-down', function (ev) {
      ...
      if ($(this).closest('.filters-select-wizard').length > 0 || $(this).closest('.filter').find('.filters-modal-wizard').length > 0) {
        ...
        if (prevV === 1 && nextV === 1 && !flag) {
          ...
          $(this).closest('.product__single-item-info-bottom').find('.product__single-item-info-price').data('single-price', $(this).closest('.product__single-item-info-bottom').find('.product__single-item-info-price').data('single-price') * 1 / (flag ? $(this).closest('.quantity').find('input.product-qty').val() * 1 - 1 : $(this).closest('.quantity').find('input.product-qty').val() * 1 + 1) * ($(this).closest('.quantity').find('input.product-qty').val() * 1)); // $(this).closest('.product__single-item-info-bottom').find('.product__single-item-info-price span').html(`${getCurrencySymbol()}${$(this).closest('.product__single-item-info-bottom').find('.product__single-item-info-price').data('single-price')}`);
          // setTotalPrice(countTotalPrice());
          // return true;
        }
        ...
        variation_id = $(this).closest('.product__single-item-info-bottom').find('.custom-control-input').val(); // console.log($(this).closest('.product__single-item-info-bottom').find('.product__single-item-info-price').data('single-price')*1, ($(this).closest('.quantity').find('input.product-qty').val()*1-1), $(this).closest('.quantity').find('input.product-qty').val()*1);
      ...
      fetch("/products/get-discount-price", {
        ...
        // alert(data.price)
        console.log(333333, '--------');
        price_place.html("".concat(getCurrencySymbol()).concat(data.price));
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (!data.error) {
            ...
            setTotalPrice(countTotalPrice()); // get_price();
            // call_subtotal();
          }
        }
      });
    ...
    $('body').on('click', '#singleProductPageCnt .product__single-item-add-new a.product__single-item-add-new-btn', function (ev) {
      ...
      checkLimit($self.closest('.product__single-item-info.limit').find('.product__single-item-info-bottom').length + 1, $self.closest('.product__single-item-info.limit').data('limit')) && fetch("/products/get-variation-menu-raw", {
        ...
        row.find('.product__single-item-add-new').before(data.html); // console.log(row.find('.select-2'));

        row.find('.product__single-item-info-bottom').last().find('.select-2').select2({
          minimumResultsForSearch: -1
        });
        $self.closest('.product__single-item-info').css('border-color', '#d7d7d7');
        setTotalPrice(countTotalPrice()); // const new_rows_list = row.find('.product__single-item-info-bottom');
    ...
    $('body').on('change', '#singleProductPageCnt .product_radio-single .custom-radio .custom-control-input[type="radio"]', function (ev) {
      ...
      fetch("/products/get-variation-menu-raw", {
        ...
        $self.closest('.product__single-item-info').css('border-color', '#d7d7d7'); // row.find('.product-qty').select2();
    ...
    $('body').on('click', '.special__popup-main-product-item-btn.add-btn', function () {
      ...
      if (wrongMinLimit.length === 0) {
        fetch("/products/add-offer", {
          ...
          $self.closest('.special__popup-main-product-item').addClass('user-non-select'); // console.log($self.closest('.special__popup-main-product-item'));

          btnAddToRemove($self);

          if ($("#specialPopUpModal .added-offers .special__popup-content-right-product[data-id=\"".concat(id, "\"]")).length === 0) {
        ...
        wrongMinLimit.map(function (groupId) {
          $self.closest('.special__popup-main-product-item').find(".limit.pr-wrap[data-group-id=\"".concat(groupId, "\"]")).css('border', '1px solid red');
        });
      }
    });
    $('body').on('click', '.special__popup-main-product-item-btn.remove-btn', function () {
      ...
      var id = $(this).closest('.special__popup-main-product-item').data('id');
      $('.special__popup-content-right-item.added-offers').find(".special__popup-content-right-product[data-id=\"".concat(id, "\"]")).remove();
      countOfferTotalPrice();
    });
    $('body').on('click', '.special__popup-content-right-product-remove', function () {
      ...
      var product = $("#specialPopUpModal .special__popup-main-product-item[data-id=\"".concat(id, "\"]"));
    ...
    var addDataKey = {}; //item price

    var item_price = 0; //section price

    var section_price = 0; //extra group ids

    var selectedGroupId = []; //event default
    ...
    var handleProductCountPlus = function handleProductCountPlus(plus_button, section, type, limit) {
      var counter = $(plus_button.closest('.continue-shp-wrapp_qty').find('.field-input')[0]); // new_qty(section);
      // Number(counter.val()) < Number(limit) - Number(new_qty(section)) +

      Number($(plus_button.closest('.continue-shp-wrapp_qty').find('.field-input')[0]).val()) && counter.val(Number(counter.val()) + 1); // new_qty(section);
      // if (type === 'select') {
      //     select2MaxLimit(section, limit);
      // }

      var price = plus_button.closest('[data-price]').attr('data-price');
      plus_button.closest('[data-price]').find('.price-placee').html("".concat(getCurrencySymbol()).concat(price * Number(counter.val())));
    ...
    var makeOutOfStockSelectOption = function makeOutOfStockSelectOption(select, type) {
      if (type === "select") {
        ...
        var current_item_id = $(select.find('[data-out="0"]')[0]).attr('data-select2-id'); // new_qty(select);
    ...
    var discountInputChange = function discountInputChange($ev, $element, discount_type) {
      ...
      var variation_id = $element.attr('data-id'); // console.log(variation_id);
      ...
      if (discount_type === 'range') {
        ...
        fetch("/products/get-discount-price", {
          ...
          $element.closest('.menu-item-selected').find('.price-placee').html("".concat(getCurrencySymbol()).concat(data.price));
        })["catch"](function (error) {
          return console.error(error);
        });
    ...
    var unselectHandle = function unselectHandle(select, id) {
      select.closest('.filters-select-wizard').find(".product__single-item-info-bottom[data-id=\"".concat(id, "\"]")).remove();
      setTimeout(function () {
    ...
    var selectHandle = function selectHandle(el, id, selectElementId, limit, select) {
      ...
      fetch("/products/get-variation-menu-raws", {
        ...
        el.closest('.product__single-item-info-bottom').find('.filter-children-items').append(json.html); // select2MaxLimit(select, limit);

        setTotalPrice(countTotalPrice()); // el.closest('.product__single-item-info-bottom').find('.filter-children-items').find('.product__single-item-info-bottom').remove();
    ...
    var selectOfferHandle = function selectOfferHandle(el, id, selectElementId, limit, select) {
      ...
      fetch("/products/get-offer-menu-raws", {
        ...
        // const isMultiple = select.closest('[data-limit]').attr('data-limit') === '1' ? false : true;
        el.closest('.filter-children-items').append(json.html); // select2MaxLimit(select, limit);
        // setTotalPrice(countTotalPrice());
        ...
        countOfferPrice();
        countOfferTotalPrice(); // setTotalPrice(modal);
      })["catch"](function (error) {
        console.log(error);
      });
    ...
    function limite_message(group_id, active_item, discount) {
      ...
      var limit = $(".product__single-item-info[data-group-id=\"".concat(group_id, "\"]")).data('limit');
      var min_limit = $(".product__single-item-info[data-group-id=\"".concat(group_id, "\"]")).data('min-limit');
      var count = 0;
      ...
      var message = '';
      console.log(22222222222, count); // console.log(count, min_limit, limit)
      ...
      if (limit !== 1) {
        if (min_limit >= 1 && count === 0) {
          ...
          message = "".concat(min_limit - count, " items left");
        } else if (count === limit && !active_item) {
          message = "You allowed to select ".concat(limit, " items only");
    ...
    function activate_item(self, id, name, group_id, discount) {
      ...
      if (limit !== 1) {
        if ($(self).hasClass('active')) {
          $("#wizardViewModal #myTabContent").find("li[data-id=\"".concat(id, "\"]")).each(function () {
            $(this).find('.item-content').removeClass('active');
          });
          $('#wizardViewModal .footer-list').find("li[data-id=\"".concat(id, "\"]")).remove();
        } else {
          var group_element = $(".product__single-item-info[data-group-id=\"".concat(group_id, "\"]"));
          ...
          if (discount) {
            ...
            if (count + currentCount <= group_element.data('limit')) {
              ...
              $('#wizardViewModal .footer-list').find(".footer-list-item[data-id=\"".concat(id, "\"]")).length === 0 && $('#wizardViewModal .footer-list').append("<li class=\"footer-list-item\" data-id=\"".concat(id, "\" data-name=\"").concat(name, "\">\n                                                                <span class=\"title\">").concat(name, "</span>\n                                                                <span class=\"close-icon item-selected-footer\"><i class=\"fa fa-times\"></i></span>\n                                                            </li>"));
            }
          } else {
        ...
        var _group_element = $(".product__single-item-info[data-group-id=\"".concat(group_id, "\"]"));
        ...
        if ($("#wizardViewModal #myTabContent #wizardAll").find('.item-content.active').length < _group_element.data('limit')) {
          ...
          $('#wizardViewModal .footer-list').find(".footer-list-item[data-id=\"".concat(id, "\"]")).length === 0 && $('#wizardViewModal .footer-list').html("<li class=\"footer-list-item\" data-id=\"".concat(id, "\" data-name=\"").concat(name, "\">\n                                                            <span class=\"title\">").concat(name, "</span>\n                                                            <span class=\"close-icon item-selected-footer\"><i class=\"fa fa-times\"></i></span>\n                                                        </li>"));
        } // }
    ...
    var filterModalSingleInit = function filterModalSingleInit() {
      (function () {
        $("#singleProductPageCnt .filters-modal-wizard").each(function (index) {
          ...
          selected_ides = [];
          var x_group;
          $("body").on('click', ".filters-modal-wizard[data-group=\"".concat(button_group_id, "\"]"), function () {
            var group_id = $(this).data('group');
            x_group = group_id;
            $("#wizardViewModal").attr('data-group', group_id); // const selectedIds = $(this).closest('.product-single-info_row').find('.menu-item-selected').toArray().map(function (item) {
            //     return $(item).attr('data-id');
            // });
            ...
            $.ajax({
              ...
              success: function success(data) {
                ...
                $(".product__single-item-info[data-group-id=\"".concat(group_id, "\"]")).find('.product__single-item-info-bottom').each(function (a, b) {
                    ...
                    if (selected_ides.includes($(this).data('id'))) {
                      ...
                      $("#wizardViewModal ul.content li").each(function () {
                        ...
                        if (active_item) {
                          ...
                          $(this).find('.product-qty-select').val($(".product__single-item-info.filter_discount[data-group-id=\"".concat(button_group_id, "\"]")).find(".product__single-item-info-bottom[data-id=\"".concat(selectedItemIdDiscount, "\"]")).find('.product-qty_per_price').val());
          ...
          $('body').on('click', "#wizardViewModal[data-group=\"".concat(button_group_id, "\"] .close-icon.item-selected-footer"), function (ev) {
            var id = $(this).closest('li').data('id');
            var group_id = button_group_id;
            $("#wizardViewModal #myTabContent").find("li[data-id=\"".concat(id, "\"]")).each(function () {
              $(this).find('.active').removeClass('active');
            });
            $(this).closest('li').remove(); // console.log(x_group, group_id)

            if (x_group === group_id) {
              limite_message(x_group);
            }
          });
          $('body').on('click', "#wizardViewModal[data-group=\"".concat(button_group_id, "\"] .b_save"), function () {
            var items_array = [];
            var self = $(this);
            console.log('lalalalalala');
            var isDiscount = $(".product__single-item-info[data-group-id=\"".concat(self.closest('#wizardViewModal').data('group'), "\"]")).hasClass('filter_discount');
            ...
            if (isDiscount) {
              ...
              var _popup_items_qty = []; // console.log($(`[data-id-popup].selected-item_popup`).find('.popup_field-input'));
              ...
              var _popup_items_qty2 = []; // console.log($(`[data-id-popup].selected-item_popup`).find('.popup_field-input'));
            ...
            fetch("/products/get-variation-menu-raws", {
              ...
              credentials: "same-origin",
              body: isDiscount ? JSON.stringify({
                ids: items_array,
                type: 'discount'
              }) : JSON.stringify({
                ids: items_array
              ...
              // console.log(json);
              var items_row = $("[data-group-id=\"".concat(button_group_id, "\"]")).find('.product-single-info_row-items');
              ...
              $("[data-group-id=\"".concat(button_group_id, "\"]")).closest('.product__single-item-info').css('border-color', '#d7d7d7');
              $("[data-group-id=\"".concat(button_group_id, "\"]")).closest('.product-single-info_row').find('.field-input').each(function () {
              ...
              setTotalPrice(countTotalPrice());
              $('#wizardViewModal').modal('hide');
              $("[data-group=\"".concat(button_group_id, "\"]")).closest('.product-single-info_row').on('click', '.delete-menu-item', function () {
                $(this).closest('.menu-item-selected').remove();
                setTotalPrice(countTotalPrice());
              });
              $("[data-group=\"".concat(button_group_id, "\"]")).closest('.product-single-info_row').on('click', '.product-count-minus', function (ev) {
                ev.preventDefault();
                ev.stopImmediatePropagation();
                var limit = $(this).closest('.product-single-info_row').find('.limit[data-limit]').attr('data-limit');
                handleProductCountMinus($(this), $("[data-group=\"".concat(button_group_id, "\"]")), 'popup', limit);
                setTotalPrice(countTotalPrice());
              });
              $("[data-group=\"".concat(button_group_id, "\"]")).closest('.product-single-info_row').on('click', '.product-count-plus', function (ev) {
                ev.preventDefault();
                ev.stopImmediatePropagation();
                var limit = $(this).closest('.product-single-info_row').find('.limit[data-limit]').attr('data-limit');
                handleProductCountPlus($(this), $("[data-group=\"".concat(button_group_id, "\"]")), 'popup', limit);
                setTotalPrice(countTotalPrice());
          ...
          $('body').on('click', '#wizardViewModal .item-selected-footer', function () {
            ...
            activate_item($("#wizardViewModal .content[data-id=\"".concat(id, "\"]")).find('.item-content'), id, name);
      ...
      (function () {
        $("#specialPopUpModal .filters-modal-wizard").each(function (index) {
          ...
          $("body").on('click', ".filters-modal-wizard[data-group=\"".concat(button_group_id, "\"]"), function () {
            ...
            $.ajax({
              ...
              success: function success(data) {
                $("#wizardViewModal .modal-body").html(data.html);
                selected_ides.length = 0;
                $(".product__single-item-info[data-group-id=\"".concat(button_group_id, "\"]")).find('.product__single-item-info-bottom').each(function (a, b) {
                  $(this).data('id') && selected_ides.push($(this).data('id'));
                });
                $("#wizardViewModal ul.content li").each(function () {
                  $(this).find(".item-content").on('click', function () {
                ...
                limite_message(group_id, true); // $(`#wizardViewModal ul.content li`).each(function() {
                //
                // });
          ...
          $('body').on('click', "#wizardViewModal[data-group=\"".concat(button_group_id, "\"] .b_save"), function () {
            ...
            var popup_items_qty = []; // console.log($(`[data-id-popup].selected-item_popup`).find('.popup_field-input'));
    ...
    var filterSelectSingleInit = function filterSelectSingleInit() {
      (function () {
        $("#singleProductPageCnt .filters-select-wizard").each(function () {
          ...
          $("[data-group=\"".concat(group_id, "\"]")).on('change', function () {
            var self = $(this);
              ...
              if (!res.error) {
                switch (res.type) {
                  ...
                  case 'items':
                    ...
                    if (isMultiple) {// select2MaxLimit(parentRow.find('.product--select-items'), limit);
                    } else {
                      setTimeout(function () {
          ...
          $("[data-group=\"".concat(group_id, "\"]")).on('select2:select', '.product--select-items', function (e) {
            var id = e.params.data.id;
            var limit = $(this).closest('[data-limit]').attr('data-limit');
            var selectElementId = $(e.params.data.element).attr('data-select2-id'); // console.log(1111111111111, e.params);

            selectHandle($(e.target), id, selectElementId, limit, $(this));
          });
          $("[data-group=\"".concat(group_id, "\"]")).on('select2:unselect', '.product--select-items', function (e) {
            // console.log(e)
            // const limit = $(this).closest('[data-limit]').attr('data-limit');
            unselectHandle($(this), e.params.data.id);
          });
          $("[data-group=\"".concat(group_id, "\"]")).on('click', '.product-count-minus', function (ev) {
            eventInitialDefault(ev);
            var limit = $(this).closest('[data-limit]').attr('data-limit');
          ...
          $("[data-group=\"".concat(group_id, "\"]")).on('click', '.product-count-plus', function (ev) {
            eventInitialDefault(ev);
            var limit = $(this).closest('[data-limit]').attr('data-limit');
            var row = $(this).closest('.product-single-info_row');
            var select = row.find('.product--select-items');
          ...
          $("[data-group=\"".concat(group_id, "\"]")).on('click', '.remove-single_product-item', function () {
            // const limit = $(this).closest('[data-limit]').attr('data-limit');
            if ($(this).closest('.filters-select-wizard').length > 0) {
              var $this = $(this);
              var s_id = $this.attr('data-el-id');
              $(".select2-selection__choice[data-select2-id=\"".concat(s_id, "\"].select2-selection__choice__remove")).click();
              $(this).closest('.filters-select-wizard').find("option[data-select2-id=\"".concat(s_id, "\"]"));
              ...
              $this.closest('.menu-item-selected').remove(); // select2MaxLimit($(this).closest('.filters-select-wizard').find('.product--select-items'), limit);
      ...
      if (location.pathname === "/my-cart") {
        if (activeProducts.length > 0) {
            ...
            pr_items.each(function () {
              ...
              $(this).find('.product__single-item-info-bottom').each(function () {
                ...
                if ($(this).closest('.filter').length > 0 && $(this).hasClass('get-single-price')) {
                  ...
                  id = $(this).find('.custom-control-input:checked').val(); // console.log('id', id, $(this), $(this).find('.custom-control-input:checked'),  555555555);
          ...
          fetch("/add-extra-to-cart", {
            ...
            // btnAddToRemove($self);
            // $('.special__popup-content-right-item.added-offers').append(data.html);
            $(".cart-area").html(data.html); // addOfferEvent();

            $("#specialPopUpModal").modal('hide'); // $('#headerShopCartBtn').click();
          })["catch"](function (error) {
            console.log(error);
          });
        } else {}
    ...
    $("body").on('click', ".popup-select", function () {
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (!data.error) {
            $("#popUpModal .modal-content").html(data.html); // $('#popUpModal .title_popup').text(`You can add ${limit} product`);
    ...
    $("body").on('click', "#popUpModal .single-item-wrapper .single-item", function (ev) {
      ...
      var selectedItemsCountInPage = $('#singleProductPageCnt').find("[data-group-id=\"".concat(group, "\"]")).find('.product__single-item-info-bottom').length || $('#specialPopUpModal').find("[data-group-id=\"".concat(group, "\"]")).find('.product__single-item-info-bottom').length;
      console.log({
        id: id,
        title: title,
      ...
      if (!$(this).closest(".single-item-wrapper").hasClass('active') && selectedCount + 1 + selectedItemsCountInPage <= limit) {
        $(this).closest(".single-item-wrapper").addClass('active'); // $(this).closest('.modal').find('.selected-items_popup')
    ...
    $("body").on('click', "#popUpModal .modal-footer .b_save", function () {
      ...
      $('#popUpModal').find('.single-item-wrapper.active').each(function () {
        items_value_array.push({
          id: $(this).data('id'),
          value: 1 // $(this).find('.selected-item-popup_qty-select').val()
      ...
      if ($(this).closest('#popUpModal').attr('product-or-offer') === 'product') {
        fetch("/products/get-variation-menu-raws", {
          ...
          var selected_product_wrapper = $("[data-group=\"".concat(dg_popup, "\"]")).closest('.product-single-info_row').find('.product-single-info_row-items');
          $(".product__single-item-info[data-group-id=\"".concat(dg_popup, "\"]")).append(json.html);
          $(".product__single-item-info[data-group-id=\"".concat(dg_popup, "\"]")).find('.select-2').each(function () {
            $(this).select2({
              minimumResultsForSearch: -1
            });
          });
          ...
          json.items.map(function (item) {
            var item_price = Number(selected_product_wrapper.find(".menu-item-selected[data-id=\"".concat(item.id, "\"]")).attr('data-price'));
            selected_product_wrapper.find(".menu-item-selected[data-id=\"".concat(item.id, "\"]")).find('.product-qty').val(Number(item.value));
            selected_product_wrapper.find(".menu-item-selected[data-id=\"".concat(item.id, "\"]")).find('.price-placee').html("".concat(getCurrencySymbol()).concat(item_price * Number(item.value)));
          });
          $(".product__single-item-info[data-group-id=\"".concat(dg_popup, "\"]")).closest('.product__single-item-info').css('border-color', '#d7d7d7'); // setTotalPrice(modal);

          setTotalPrice(countTotalPrice());
          $('#popUpModal').modal('hide');
          $("[data-group=\"".concat(dg_popup, "\"]")).closest('.product-single-info_row').on('click', '.delete-menu-item', function () {
            $(this).closest('.menu-item-selected').remove();
            setTotalPrice(countTotalPrice());
        ...
        fetch("/products/get-offer-menu-raws", {
          ...
          var selected_product_wrapper = $(".pr-wrap[data-group-id=\"".concat(dg_popup, "\"]")); // console.log(111111111111111111, selected_product_wrapper, dg_popup)

          selected_product_wrapper.append(json.html); // $(`.product__single-item-info[data-group-id="${dg_popup}"]`).append(json.html);

          $(".pr-wrap[data-group-id=\"".concat(dg_popup, "\"]")).find('.select-2').each(function () {
            $(this).select2({
              minimumResultsForSearch: -1
            });
          }); // $(this).closest('.product__single-item-info-bottom').find('.get-single-price').data('single-price', data.price)

          countOfferPrice(); // selected_product_wrapper.find('.product__single-item-info-bottom').remove();
          //
          //
          // json.items.map((item) => {
    ...
    $("body").on('click', '.btn-add-to-cart', function (e) {
      ...
      variations.map(function (gr) {
        if (gr.limit) {
          ...
          var minLimit = $('#singleProductPageCnt').find("[data-group-id=\"".concat(gr.group_id, "\"]")).attr('data-min-limit') * 1;
          var maxLimit = $('#singleProductPageCnt').find("[data-group-id=\"".concat(gr.group_id, "\"]")).attr('data-limit') * 1;
      ...
      if (bad.length !== 0) {
        bad.map(function (group_id) {
          $(".product__single-item-info[data-group-id=\"".concat(group_id, "\"]")).css('border-color', 'red');
        });
        return false;
      } else {
        $.ajax({
    ...
    $("body").on("click", ".extra-sections", function () {
      ...
      AjaxCall("/products/get-extra-content", {
        id: id
      }, function (res) {
        if (!res.error) {
          $("#extraModal .modal-body").html(res.html);
    ...
    $('.shopping-cart-inner').find('.product-qty-select').addClass('none-touchable'); // $("body").on('click', '.qty-count', function (ev) {
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (!data.error) {
            ...
            $('#specialPopUpModal .modal-body').find('.select-2').select2({
              minimumResultsForSearch: -1
            });
            $('.special__popup-content-right-product').each(function () {
              if ($(".special__popup-main-product-item[data-id=\"".concat($(this).data('id'), "\"]")).length > 0) {
                $(".special__popup-main-product-item[data-id=\"".concat($(this).data('id'), "\"]")).addClass('user-non-select');
              }
            });
            ...
            $("#specialPopUpModal").modal(); // if(data.message === 'added') {
            //     $('#cartSidebar').html(data.headerHtml);
            //     $('.add-cart-number.cart-count').html(data.count);
            //     $('#specialPopUpModal .modal-body').html(data.specialHtml);
            //     $('.special__popup-main-product-item .select-2').each(function() {
...
localStorage.getItem('display-grid') == "list" && $('#displVertBtn').click(); // scroll top button
...
$('body').on('click', '.range-steps_count', function () {
  ...
  $(this).closest('.range-steps').find('input').removeAttr('checked'); // if(!rangeItem.find('input').is(":checked")){
...
changeHeaderWhenIsCookie(); // display filter for mobile
...
openSidebar($('.share-button.facebook-share-button'), $('#share_modal')); // my account select make fixed when scrolled
...
$('body').on('click', function (ev) {
  ...
  if ($(ev.target).closest('.navbar-toggler').length === 0) {
    if ($('.navbar-collapse').hasClass('show') && !($(ev.target).hasClass('navbar-nav') || $(ev.target).closest('.navbar-nav').length > 0)) {
      $('.navbar-collapse').removeClass('show'); // console.log(1);
    }
  }
}); // $("body").on('click', `#popUpModal .modal-footer .b_save`, function () {
